...
namespace DelvUI.Interface.GeneralElements
{
        ...
        : DraggableHudElement(config, displayName), IHudElementWithActor, IHudElementWithMouseOver, IHudElementWithPreview, IHudElementWithVisibilityConfig
    {
        ...
        public override void DrawChildren(Vector2 origin)
        {
            ...
            if (Config is PlayerUnitFrameConfig)
            {
                var orbConfig = ConfigurationManager.Instance.GetConfigObject<PlayerParameterOrbConfig>();
                if (orbConfig != null && orbConfig.Enabled && orbConfig.HideDefaultPlayerUnitFrame)
                {
                    StopMouseover();
                    return;
                }
