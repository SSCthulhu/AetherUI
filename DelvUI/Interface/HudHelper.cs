...
namespace DelvUI.Interface
{
    public class HudHelper : IDisposable
    {
        ...
        private bool _lastMinimapEnabled = false;
        ...
        protected void Dispose(bool disposing)
        {
            ...
            if (Plugin.Framework.IsInFrameworkUpdateThread && Plugin.ObjectTable.LocalPlayer != null)
            {
                ...
                UpdateNativeMinimapVisibility(true);
            }
            else
            {
                try
        ...
        private void UpdateNativeMinimapVisibility(bool forceVisible = false)
        {
            bool minimapEnabled = !forceVisible && ConfigurationManager.Instance.GetConfigObject<MinimapConfig>()?.Enabled == true;

            if (forceVisible)
            {
                NativeMinimapVisibility.SetVisible(true);
                _lastMinimapEnabled = false;
                return;
            }

            if (_lastMinimapEnabled == minimapEnabled)
            {
                return;
            }

            NativeMinimapVisibility.SetVisible(!minimapEnabled);
            _lastMinimapEnabled = minimapEnabled;
        }
